$(document).ready(function(){
	...
	$("span.thumb").bind("mousemove", function(e) {
		...
		var length = (this.getAttribute("id")[3]);
	...
	$('div.overlay').bind("click", function(e) {
...
var overlay = {
	...
	prepare: function(cur) {
		...
		var img = $("<img class='cur' src='"+cur.src+"' />").attr("src", cur.getAttribute('id').replace(/_/g, '/')).appendTo("div.overlay");
		...
		var pos_x = (cw / 2) - ( $(img)[0].offsetWidth / 2 ) + 50;
		$(img).css({ left: pos_x, top: (ch / 2) - ( $(img)[0].offsetHeight / 2 ) });

		//Create the left hand image
		var img_left = $("<img class='prev' id='"+(prev ? prev.getAttribute('id') : cur.getAttribute('id').replace(/_/g, '/'))+"' src='"+(prev ? prev.src : cur.src)+"' style='width: 50px;' />").appendTo(overlay.container);
		img_left.css({ left: (pos_x / 2) - ( 50 / 2 ), top: (ch / 2) - ( img_left[0].offsetHeight / 2 ) });
		if(!prev) img_left.css("visibility", "hidden"); else img_left[0].src = prev.getAttribute('id').replace(/_/g, '/'); //Hide this one if it's only a filler, otherwise lazy load the pic

		//Create the right hand image
		var img_right = $("<img class='next' id='"+(next ? next.getAttribute('id') : cur.getAttribute('id').replace(/_/g, '/'))+"' src='"+(next ? next.src : cur.src)+"' style='width: 50px;' />").appendTo(overlay.container);
		$(img_right).css({ left: (pos_x+$(img)[0].offsetWidth) + (pos_x / 2) - ( 50 / 2 ), top: (ch / 2) - ( $(img_right)[0].offsetHeight / 2 ) });
		if(!next) img_right.css("visibility", "hidden"); else img_right[0].src = next.getAttribute('id').replace(/_/g, '/'); //Hide this one if it's only a filler, otherwise lazy load the pic
		...
		var cur_clone = $(cur)
			...
			.appendTo($(cur).parent())
			.css(pos)
			.css({ position: "absolute", opacity: 1, visibility: "visible", padding: "5px", marginLeft: "0", marginTop: "0", zIndex: 1000 })
			.animate({ top: (ch / 2) + $(cur).parent()[0].scrollTop - ( $(img)[0].offsetHeight / 2 ) - 25, left: -85, width: img.width(), height: img.height() }, 500, function() {
	...
	next: function() {
		...
		var upcoming = $("div.right:visible img[@src*="+strippedsrc+"]").next().not(".bigthumb");
		
		var img_right = $("<img class='next' src='"+(upcoming.length ? upcoming[0].src : cur[0].src)+"' style='width: 50px; visibility: hidden;' />").appendTo("div.overlay");
		...
		if(upcoming.length)
			img_right.animate({ width: 50, top: curNextPosition[1], left: curNextPosition[0] }, overlay.speed, function() { this.src = upcoming[0].getAttribute("id").replace(/_/g, '/'); });	
	...
	prev: function() {
		...
		var strippedsrc = path[path.length-1];
		var upcoming = $("div.right:visible img[@src*="+strippedsrc+"]").prev().not(".bigthumb");
		
		var img_left = $("<img class='prev' src='"+(upcoming.length ? upcoming[0].src : cur[0].src)+"' style='width: 50px; visibility: hidden;' />").appendTo("div.overlay");
		...
		if(upcoming.length)
			img_left.animate({ width: 50, top: curPrevPosition[1] }, overlay.speed, function() { this.src = upcoming[0].getAttribute("id").replace(/_/g, '/'); });	
